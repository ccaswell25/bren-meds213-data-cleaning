...
```{r}
snowsurvey_csv %>% 
  count(Snow_cover) #count of all values that exist in our snow cover column. we have "-s" and "."s 
```
...
snowsurvey_csv %>% 
  count(Snow_cover) %>% 
  filter(is.na(as.numeric(Snow_cover))) #converting snow_cover result to numeric and then determining those that are nas / cant be numeric
```
...
snowsurvey_fixed <- snowsurvey_csv %>% 
 # filter(Snow_cover == ".") %>% commented out to test it worked first
  mutate(Snow_cover = ifelse(Snow_cover == ".", NA, Snow_cover))#replacing "."s with NAs
```
...
glimpse(snowsurvey_fixed) #now it is dbl
```
...
```{r}
 snowsurvey_fixed %>% 
  filter(Snow_cover > 100) #we have values above 100%...need to fix this
```
...
#257 columns because each species is its own column 
```
...
species_long <- species_csv %>% 
  pivot_longer(
    cols = !c(Year, Site, Date, Jdate, Num_observers, All_obs_reported, Observer_hours),#excluding the columns that aren't species columns
    names_to = "species",
    values_to = "species_count",
    values_transform = list(species_count = as.character) #transforming new species count column to character
  )
```
...
species_presence %>% 
  ...
  count(species) %>% 
  arrange(desc(n)) #arranging species counts to the most to lease
```
